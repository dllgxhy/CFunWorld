...
YoungMakerBuzzer::YoungMakerBuzzer()
 {
 
 }
 
YoungMakerBuzzer::YoungMakerBuzzer(uint8_t pin)
 {
  buzzer_pin = pin;
 }
...
void YoungMakerBuzzer::tone(uint8_t pin,uint16_t frequency, uint32_t duration)
{
  buzzer_pin = pin;
  // _buzz_ir=0;
...
void YoungMakerBuzzer::noTone(uint8_t pin)
{
    buzzer_pin=pin;
