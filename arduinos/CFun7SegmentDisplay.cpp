...
CFun7SegmentDisplay::CFun7SegmentDisplay(uint8_t port):CFunPort(port)
{
	DIO = s1;
	SCLK = s2;
        RCLK= s3;
...
void CFun7SegmentDisplay::reset(uint8_t port){
        _port = port;
	s1 = cfunPort[port].s1;
	s2 = cfunPort[port].s2;
        s3 = cfunPort[port].s3;
...
void CFun7SegmentDisplay::display(double value){
  if(!CFun7SegmentDisplay_first)
  {
     redisplay(value);
     CFun7SegmentDisplay_first++;
...
void CFun7SegmentDisplay::display(int value){
  if(!CFun7SegmentDisplay_first)
  {
     redisplay(value);
     CFun7SegmentDisplay_first++;
...
int CFun7SegmentDisplay::pows(int a,int b){
  int c = 1;
  int i =0;
  for(i=0;i<b;i++)
    c *= a;
...
int CFun7SegmentDisplay::checkNum(float v,int b){
	if(b>=0){
...
//******************************************
void CFun7SegmentDisplay::display(int8_t DispData[])
{
  uint8_t i;
  for(i=0;i<4;i++){
...
void CFun7SegmentDisplay::clearDisplay(void)
{   
