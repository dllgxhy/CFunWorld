...
//		var obj:ScratchObj = interp.targetObj();
		...
		{
//			var w:DisplayObject;
//			if (obj) app.runtime.showVarOrListForWarn(interp.arg(b, 0), false, obj, w);
			DialogBox.warnconfirm(Translator.map("can't support this block,please remove it"),Translator.map("show variable"), null, app.stage);//软件界面中部显示提示框_wh
		}
		else
		{
			var obj:ScratchObj = interp.targetObj();
	...
	private function primHideWatcher(b:Block):* {
		if(app.ArduinoFlag == true)//判断是否为Arduino语句生成过程_wh
		{
			DialogBox.warnconfirm(Translator.map("can't support this block,please remove it"),Translator.map("hide variable"), null, app.stage);//软件界面中部显示提示框_wh
		}
		else
		{
			var obj:ScratchObj = interp.targetObj();
