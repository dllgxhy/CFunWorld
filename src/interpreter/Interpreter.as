...
//		checkPrims();
	...
	public function toggleThread(b:Block, targetObj:*, startupDelay:int = 0):void {
		...
		if (wasRunning) {
			...
			if (b.isReporter) {
				//对于返回数据类的积木模块，b.isReporter值为true，并增加了一个b.op = “0”的线程任务_wh
				//而对于新增加的读硬件数据模块处理不需要，故加判断_wh
	...
	public function stepThreads():void {
		...
		if (threads.length == 0)
		{
			//如果是在Arduino程序生成结尾，添加文件尾，标识清零，文件流关闭，并退出_wh
...
//					app.ArduinoFs.writeUTFBytes('#include <Wire.h>' + '\n');
				...
				if(app.ArduinoRPFlag == true)
				{
					if(app.ArduinoWarnFlag == false)
					{
						...
						{
							app.ArduinoRPNum = 0;
							app.ArduinoFile.openWithDefaultApplication();//用默认的IDE打开_wh
						}
						
						if(app.ArduinoRPNum == 1)
						{
							app.ArduinoRPNum = 0;
							app.ArduinoFile.copyTo(app.ArduinoFileB,true);//将arduinos.ino复制到ArduinoBuilder目录下_wh
...
//							var nativePSInfo:NativeProcessStartupInfo = new NativeProcessStartupInfo();
							...
							app.delay1sTimer.start();//开启定时器_wh
							app.cmdBackNum = 1;
							app.process.addEventListener(ProgressEvent.STANDARD_OUTPUT_DATA, app.cmdDataHandler);//cmd返回数据处理事件_wh	
						}
					}
				}
			}
	...
	private function stepActiveThread():void {
		...
		while (true) {
			...
			while (activeThread.block == null) { // end of block sequence
				...
				{
					if(app.ArduinoBracketFlag  > 0)// && (app.ArduinoIEBracketFlag  == 0))
					{
					...
					{
						if(app.ArduinoIEElseFlag2 == 0)//若else下不再嵌套if-else，则加尾括号_wh
						{
							while(app.ArduinoIEElseFlag > 0)
							{
								if(app.ArduinoLoopFlag == true)
									app.ArduinoLoopFs.writeUTFBytes("}" + '\n');
								else
									app.ArduinoDoFs.writeUTFBytes("}" + '\n');
								app.ArduinoIEElseFlag --;
//								if(app.ArduinoIEBracketFlag)
//									app.ArduinoIEBracketFlag --;
							}
							if(app.ArduinoIEFlag == 0)
							{
								//app.ArduinoIEFlag = 0;
				...
				if(app.ArduinoBracketFlag  > 0)// && (app.ArduinoIEBracketFlag  == 0))
				{
					app.ArduinoBracketFlag  --;
					if (!activeThread.popState()) return; // end of script
				}
				else
				{
					if(app.ArduinoIEFlag)//Arduino生成的IfElse的第二个代码块_wh
					{
						activeThread.popState(); // end of script
						var IENumPre:Number = app.ArduinoIEFlagAll;
						app.ArduinoIEElseFlag ++;
						if(app.ArduinoIEFlagIE)
							startCmdList(ArduinoIfElseB[app.ArduinoIEFlagAll - app.ArduinoIEFlag]);
						else
						{
							if(app.ArduinoIEFlagAll == 1)
								startCmdList(ArduinoIfElseB[0]);
							else
								startCmdList(ArduinoIfElseB[app.ArduinoIEFlag]);
						}
	
						app.ArduinoIEFlag --;
						if(IENumPre < app.ArduinoIEFlagAll)
							app.ArduinoIEElseFlag2 ++;
						else
							app.ArduinoIEElseFlag2 = 0;
					}
					else
						if (!activeThread.popState()) return; // end of script
				}
	...
	private function clearWarpBlock():void {
		...
		warpThread = null;
		warpBlock = null;
		app.readCDFlag = false;//通信丢失提示框标志清零_wh
	}

	//点击相应积木模块后的寻找相应函数并处理（checkBlockingArgs()检测是否还在等待数据，是则doYield()并退出等数据到达后在处理）_wh
	/* Evaluation */
	public function evalCmd(b:Block):* {
		...
		if(app.ArduinoFlag == false)//Arduino程序生成流程下不需要以下处理_wh
		{
			if(app.blueFlag)
			{
				lostTime = 250;
				if(app.tFlag)//判断是否需要加延时（蓝牙情况下）_wh
				{
				...
				lostTime = 100;
			//读取硬件数据积木模块需要先发送一次读取指令，并等待数据回馈_wh
			switch(op)
			{
				case "readdigital:":
					...
					if(activeThread.comATCOMing == 1)//该线程占用串口通信_wh)
					{
						...
						if((getTimer() - activeThread.startT) >= lostTime)//100ms长时间无回复，跳出等待_wh
						{
...
//						activeThread.comATCOMing = 1;
					...
					break;
				//case "0":break;//调试过程中会间接出现"0"，原因待查_wh
				default:break;//中间假如切换了其他模块必须重新附0_wh，前面理解错误，当赋值给其他模块时会陷入循环_wh
			}
		}
		//Arduino程序生成流程_wh
		else
...
//		}
	...
	private function startCmdList(b:Block, isLoop:Boolean = false, argList:Array = null):void {
		if(activeThread.comWaitNum == 0)//对于读取模块，由于需要循环等待，过程中不显示_wh
		{
			if (b == null) {
...
//																	app.ArduinoIEBracketFlag ++;
																...
																app.ArduinoMathNum = 0;
																app.ArduinoIEFlagAll ++;
																startCmdList(b.subStack1);
																if(app.ArduinoIEFlagAll > (app.ArduinoIEFlag2+1))
																	app.ArduinoIEFlagIE = true;
																ArduinoIfElseB[app.ArduinoIEFlag ++] = b.subStack2;//在stepActiveThread中处理_wh
																app.ArduinoIEFlag2 ++;
															}
															else
															{
	...
	protected function primVarSet(b:Block):Variable {
		...
		//如果未定义，则不赋新值，主要针对读取类积木模块循环等待用_wh
		var checkvalue:*;
		checkvalue = arg(b, 1);
