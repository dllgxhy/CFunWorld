...
//----------
	...
	public function selectProjectFile():void {
		...
		//Scratch.loadSingleFile(fileLoadHandler, [filter2])
		Scratch.loadSingleFile(fileLoadHandler, [filter2, filter1])
	}
	
	//双击打开工程文件_wh
	/******************************************************************************/
	public var file:File;
	public var fileName:String, data:ByteArray;
	public var flag:Boolean = true;
	public function fileLoadHandler(event:Event):void {
		//var file:FileReference = FileReference(event.target);
		fileName = file.name;
		data = file.data;
		doInstall();
	...
	public function doInstall(ignore:* = null):void {
		installProjectFromFile(fileName, data);
	}
	
	public function initProjectFile(str:String):void {
		// Prompt user for a file name and load that file.
		file = new File(str);
		stopAll();
	...
	protected function installProject(project:ScratchStage):void {
		...
		/*SCRATCH::allow3d _wh*/ { if(app.isIn3D) app.render3D.setStage(project, project.penLayer); }
		...
		app.extensionManager.step();
		app.projectLoaded();
		/*SCRATCH::allow3d _wh*/ { checkForGraphicEffects(); }
	}
...
//	//警告框中嵌入_wh
//	public function showVarOrListForWarn(varName:String, isList:Boolean, targetObj:ScratchObj, w:DisplayObject):void {
//		w = watcherForList(targetObj, varName);
//	}
