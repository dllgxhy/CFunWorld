...
public class PaletteSelector extends Sprite {
	...
	private static const categories:Array = [
		...
		'Events', 'Control', 'Sensing', 'Operators', 'More Blocks','Arduino Robot']; // column 2//增加Arduino Blocks模块_wh
	...
	private function initCategories():void {
		const numberOfRows:int = 6;//原先为5，由于加入Arduino Blocks修改成6
		const w:int = 275;//Script下Block区域宽度_wh
		const startY:int = 3;
		var itemH:int;
		var x:int, i:int;
		var y:int = startY;
