...
CFunTemperature::CFunTemperature():CFunPort(){
}
CFunTemperature::CFunTemperature(uint8_t port):CFunPort(port){
	_port = port;
	s1 = cfunPort[port+12].s1;
        LM35=s1;
        pinMode(LM35,INPUT);
}
void CFunTemperature::reset(uint8_t port){
        _port = port;
	s1 = cfunPort[port+12].s1;
        LM35=s1;
        pinMode(LM35,INPUT);
}
float CFunTemperature::temperature(){
